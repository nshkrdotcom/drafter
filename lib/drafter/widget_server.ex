defmodule Drafter.WidgetServer do
  ...
  def handle_cast({:event, event}, state) do
    case apply(state.module, :handle_event, [event, state.state]) do
      {:ok, new_widget_state} ->
        ...
        {:noreply, %{state | state: new_widget_state}}
  ...
  def handle_call({:update_rect, rect}, _from, state) do
    new_widget_state = if function_exported?(state.module, :on_rect_change, 2) do
      apply(state.module, :on_rect_change, [rect, state.state])
    else
      state.state
    end

    {:reply, :ok, %{state | rect: rect, state: new_widget_state}}
  end
  ...
  def handle_info(msg, state) do
    result = if function_exported?(state.module, :handle_info, 2) do
